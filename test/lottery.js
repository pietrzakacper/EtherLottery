...
contract('Lottery', accounts => {
  ...
  it('Should create lottery correctly', () => {
      Lottery.deployed()
        ...
        .then(() => LotteryInstance.createLottery(web3.toWei(5, "ether")))
    ...
    const balanceBefore = web3.fromWei(web3.eth.getBalance(oddUser)).toNumber()
      ...
      LotteryInstance.claimPrize(lotteryId, {from: oddUser})
        .then(() => {
          ...
          const prizeValue = web3.fromWei(2 * betValue)
          
          //Take gas into account
          assert.equal(Math.floor(balanceAfter - prizeValue), Math.floor(balanceBefore), 'Odd users balance should increase by twice the bet value')
        })
  })
})
